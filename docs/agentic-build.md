...
| 3 | Hard gates (hooks) | **DONE_WITH_CONCERNS — awaiting Checkpoint 3** | 2026-07-05 | On `setup/03-hooks`. Five PowerShell scripts (deny / git-guard / block-merge / tests-green / spec-freeze) + `settings.json` wiring + `gh pr merge` frontmatter guard on the 4 Bash roles. **Every script unit-proven standalone** (allow/deny matrix). **Live in-session activation blocked: this session does not execute project hooks — needs a restart to load them (DEC-23).** **Resume attempt 2026-07-05: re-ran standalone matrix 15/15 PASS + tests-green green; a live orchestrator `git merge` probe still passed straight through → hooks are NOT loaded in this session either. A genuine Claude Code restart (not `/build-resume` in-place) + hook review is required before the live suite can run.** |
