...
| 2 | Role subagents | **DONE — awaiting Checkpoint 2** | 2026-07-05 | Five roles written to `.claude/agents/` on `setup/02-roles`. Reviewer read-only (no Edit/Write). Guards fire `deny.ps1` (PowerShell) per role; proven live in Phase 3. Schema re-verified against live docs. |
