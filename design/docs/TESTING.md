...
| FR3 Marine projection | Reconnect/backoff logic (fake server), 30min de-emphasis / 2h drop windows (time-injected), AISHub-swap-in via shared adapter test | Visual: popup content, actual websocket behavior against the live aisstream service |
...
| `sources/aishub.py` | (1) returns `LayerSnapshot` with `domain=MARINE`, same shape as aisstream's — the FR3 renderer-independence assertion, done as a type/shape equality test rather than a UI test; (2) respects 1 req/min pacing. |
