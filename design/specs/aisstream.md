...
- A close-code-aware refinement — reconnect on a graceful close *only* after a session that had actually streamed data (distinguishing a bad key from a legitimate idle close) — was considered and deliberately deferred: it would require revising the acceptance test, which drains a finite fixture and awaits the read task to completion.
