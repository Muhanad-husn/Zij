...
Slice 01 (plans/store/01-land-cache.md, issue #11) implements the
`land_cache` path: `Store.init`/`close`/`get_land_cache`/`put_land_cache` and
`LandCacheRow`. Slice 02 (plans/store/02-fallback-snapshots.md, issue #40)
adds the `fallback_snapshots` path: `Store.put_fallback`/`get_fallback`
(FR8). Slice 03 (plans/store/03-config-presets.md, issue #41) adds the
