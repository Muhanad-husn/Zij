...
implementation exists, guarded by a strict xfail (DEC-33): `backend.main` has
no `create_app`/`app` yet, so the import below raises `ImportError` and the
test xfails rather than errors. Once the implementer builds `backend/main.py`
to satisfy this exact seam, the test genuinely passes (xpass) and the
test-author removes the marker to finalize the contract -- this assertion
...
@pytest.mark.xfail(
    reason="backend.main create_app/app not yet implemented [slice 01] (#17)",
    strict=True,
)
