"""Locked outer acceptance test for store slice 02 (issue #40):
...
-- config_presets (slice 03) and the cold-start repopulation policy
(scheduler slice 04) are explicitly out of scope here, per the plan.

`ZIJ_DB_PATH` points at a `tmp_path` file (never the real platformdirs
location), following the hermetic-DB convention set by
`test_store_acceptance.py` (slice 01, land_cache).
