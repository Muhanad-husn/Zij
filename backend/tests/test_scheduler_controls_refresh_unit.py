"""Unit test for the refresh-all controls (issue #55), covering:

  - "Only enabled layers appear in the refresh_all queued list."

The acceptance test (backend/tests/test_api.py::
...
a stub that hardcoded "every adapter present" would pass the acceptance test's
