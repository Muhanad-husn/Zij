"""Locked outer acceptance test for overpass-adapter slice 01 (issue #15):
...
Below the outer test are inner unit tests (DEC-34), authored against the
...
noted "test-author's plumbing choice"):
  - backend.sources.overpass.OverpassAdapter(cfg): domain=Domain.LAND,
    ...
    constructor signature `OverpassAdapter(cfg)` is this test-author's
  ...
  - backend.sources.overpass.OverpassCfg, constructible from the merged
    ...
    test-author's plumbing choice (not spec-fixed).
...
# ---------------------------------------------------------------------------
# overpass-adapter/01 (issue #15) inner units (DEC-34).
# ---------------------------------------------------------------------------
...
def _make_overpass_cfg(**overrides):
    """Minimal `OverpassCfg` for inner unit tests (test-author's plumbing
...
async def test_timeout_rotates_mirrors_then_exhausts_to_upstream_error():
    ...
    NOTE (spec-drift, issue #30): `design/specs/overpass.md` is internally
    ...
    If the founder adjudicates #30 toward immediate-UpstreamError-on-timeout,
...
# ---------------------------------------------------------------------------
# overpass-adapter/02 (issue #16) outer acceptance test (DEC-1).
# ---------------------------------------------------------------------------
#
# This is the locked behavioral contract for slice 02, transcribed from
...
# depends on the network/parsing path slice 01 already covers.
#
# Name/signature this test requires the implementer to provide (test-
...
#   ) -> list[Feature]
# The implementer is expected to also call this from inside `fetch()` (per
...
# geometry.
#
# Assumptions about Feature construction (this test-author's choices, not
...
# order, matching slice 01's parsing); point anchors use
...
# the parsing path (slice 01), never `simplify_and_cap` -- a POLYGON with a
...
def _synthetic_polygon_feature(source_id: str, attrs: dict, ring: list[list[float]]):
    ...
    test-author's plumbing choice for exercising `_simplify_geometry`'s
