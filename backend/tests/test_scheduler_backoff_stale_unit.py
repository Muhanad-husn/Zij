"""Inner unit tests for scheduler slice 03 (issue #50), transcribed from the
...
isolating gaps the single flowing outer scenario does not pin on its own:
  ...
  5. The implementer's own documented decision (backend/scheduler.py
     ...
     `spec-drift` question for the reviewer/founder, not a reason to loosen
     this test now.
...
mirroring that file's own convention and slice 01/04's precedent for
...
#      the cadence cap (implementer's documented decision).
# =============================================================================
...
async def test_upstream_backoff_capped_at_the_layers_own_cadence_not_exceeding_it():
    """Implementer's documented decision (backend/scheduler.py `_poll_loop`
