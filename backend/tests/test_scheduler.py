"""Locked outer acceptance test for scheduler slice 01 (issue #45): the
...
**Public surface this test locks (test-author's chosen minimal slice, per
the plan's "Choose the minimal constructor slice 01 can honestly support")**:
...
honest slice-01 referent yet (no write path exists), so this test constructs
`Scheduler(cfg, adapters, region)` only. `region` is a slice-01 addition (not
