...
async def test_fetch_hormuz_states(monkeypatch):
    """Locked outer acceptance test for opensky-adapter slice 02 (issue #14).
    ...
    It was authored and committed red by the test-author before `fetch()`
    existed (strict xfail, DEC-33): at that point `fetch()` unconditionally
    ...
    fixture. The implementer has since made it genuinely pass; the xfail
...
# ---------------------------------------------------------------------------
# opensky-adapter/02 (issue #14) inner units (DEC-34), authored against the
