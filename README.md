...
docs/             agentic-build.md — the build log for the workflow that built this
```
...
inside Claude Code: tool-locked role subagents (triage, spec, test, implement,
review, fix) that build and check but can never merge, with two deterministic
git hooks holding the boundary — subagents never merge to `main`, and no commit
lands on a red suite. The founder specifies and approves; the roles do the work.

That workflow is the [`agentic-engineering-org`](CLAUDE.md) setup, and its entire
supervised build — every decision, gate, and checkpoint — is logged in
[`docs/agentic-build.md`](docs/agentic-build.md). If you came here for the
