...
export function mountBadge(parent: HTMLElement, domain: BadgeDomain): Badge {
  ...
  // Toggle/Refresh are inert this slice (wiring deferred to slice 04); Caveats
  ...
  // No-op this slice — the caveat panel itself is slice 05's job (spec §5).
  caveatsButton.addEventListener('click', () => {
    console.debug(`[zij] caveats button clicked for ${domain} (panel wiring: slice 05)`);
  });
