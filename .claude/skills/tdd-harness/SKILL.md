...
> - **Gates are live (Phase 3):** subagents cannot merge (`gh pr merge`/`git merge`/plugin-merge blocked); a red `uv run pytest` blocks any commit; `design/**` is frozen during implementation. The machine can take a slice all the way to a green, reviewed PR — **and no further.**
...
| 4 | `safe-pr` | A pull request (feature branch → main) with embedded acceptance evidence (transcripts by default; Playwright for web slices) — **prepared, never merged** | Reviewer's two-stage review done; PR is open and its URL recorded in the plan; the founder merges |
