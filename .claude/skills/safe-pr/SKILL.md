...
>
> - **`safe-pr` PREPARES a PR; it never merges — and cannot.** The block-merge hook denies `gh pr merge` / `git merge` / the plugin merge tool for every subagent, and branch protection backstops it. This skill's job ends at an open, evidence-rich PR into `main`. **The founder merges** (one of the three human moments). Never attempt a merge step.
...
> - **Reviewer role, not self-certification.** In Zij the **reviewer** subagent runs the two-stage review (spec-compliance → code-quality) on the prepared PR before the founder merges; `safe-pr` assembles the evidence the reviewer and founder read. The evidence must be *real run output*, not claims.
