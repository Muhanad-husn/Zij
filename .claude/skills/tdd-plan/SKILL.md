...
> ## Zij adaptation (roles, issues, stack) — read first
> This skill runs inside the Zij enterprise (`CLAUDE.md`). Adaptations:
>
> - **Who runs it:** `tdd-plan` is **planning, not authoring** — it is driven by **triage/PM** (with the founder) to shape a slice, or by the founder during `/sprint-plan`. It writes **only** plan files under `plans/<feature-slug>/`; it writes no specs, tests, or product code.
> - **The acceptance criterion is the test-author's contract-to-be.** The Given/When/Then you write into each slice plan is the exact behavior the **test-author** will encode as the **locked outer acceptance test** (committed red before any implementation, DEC-1). Write it concrete and observable at a real endpoint so test-author can transcribe it faithfully; the **implementer** who greens it may not alter it.
> - **Issues are the system of record (DEC-4).** Each slice plan expects a **linked GitHub issue** — add an `Issue:` field (`#<n>`, or `TBD` until `/sprint-plan` files it). `/sprint-plan` files the backlog through the GitHub plugin and back-links each issue to its `plans/<feature>/<NN>-*.md`.
> - **Stack + layout.** Zij is **Python 3.13 / uv / pytest / ruff**, a **FastAPI + pipeline** product. The *project directory* is the repo root (`.`); product code lives in `backend/` (import root `zij`) and `frontend/`, tests in `backend/tests/` + `frontend/tests/`, specs/contracts in `design/`. The default slice **boundary** is an HTTP route or a CLI/pipeline invocation (web UI only for real `frontend/` slices).
