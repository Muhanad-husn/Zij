...
> ## Zij stack — pinned (skip the detection dance)
> Zij's toolchain is fixed (DEC-8), so don't re-detect it:
> - **Unit runner (inner loop):** `uv run pytest` — single file/case for speed, e.g. `uv run pytest backend/tests/test_x.py::test_case`. **Full suite before every commit:** `uv run pytest` (the tests-green hook also runs this and blocks a red commit).
> - **Lint (in REFACTOR):** `uv run ruff check` (and `uv run ruff format` if formatting).
> - **Outer/acceptance layer — non-web is the DEFAULT (see §8), not the exception.** Zij is **FastAPI + a pipeline**, so the outer test is an **integration test through the real endpoint**: the FastAPI app via `TestClient`/`httpx` asserting on status+body, or a CLI/pipeline invocation as a subprocess asserting on stdout+exit code. **Playwright §§3–4 apply ONLY to a real `frontend/` (MapLibre) web slice** — skip all Playwright setup, config, and screenshot/recording steps for the default API/pipeline slice.
> - **Evidence (default):** a captured **transcript** of the `uv run pytest` run plus a real-endpoint invocation — `safe-pr` attaches these in place of screenshots (§8).
> - **Layout:** tests in `backend/tests/` + `frontend/tests/`; product in `backend/` (import root `zij`) + `frontend/`; specs in `design/`. Project directory = repo root (`.`). **DEC-1:** the outer acceptance test is authored+locked by the **test-author**; the **implementer** drives inner cycles only and must not edit it (hooks block implementer writes under `**/tests/`).
